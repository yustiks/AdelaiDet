...
# test
labels = glob.glob('labels_test/*.txt')
labels.sort()

if not os.path.isdir('abcnet_gen_labels_test'):
            os.mkdir('abcnet_gen_labels_test')

for il, label in enumerate(labels):
    #print('Processing: '+label)
    imgdir = label.replace('labels_test/', 'images_test/').replace('.txt', '.jpg')

    outgt = open(label.replace('labels_test/', 'abcnet_gen_labels_test/').replace('.txt', '.txt'), 'w')
...
#    if not os.path.isdir('abcnet_vis_test'):
#            os.mkdir('abcnet_vis_test')
#    plt.savefig('abcnet_vis_test/'+os.path.basename(imgdir), bbox_inches='tight',dpi=400)
#    plt.clf()
#Iustina: to here
