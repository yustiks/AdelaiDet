...
# test
labels = glob.glob('labels_train/*.txt')
labels.sort()

if not os.path.isdir('abcnet_gen_labels_train'):
            os.mkdir('abcnet_gen_labels_train')

for il, label in enumerate(labels):
    # print('Processing: '+label)
    imgdir = label.replace('labels_train/', 'images_train/').replace('.txt', '.jpg')

    outgt = open(label.replace('labels_train/', 'abcnet_gen_labels_train/').replace('.txt', '.txt'), 'w')
...
#    if not os.path.isdir('abcnet_vis_train'):
#            os.mkdir('abcnet_vis_train')
#    plt.savefig('abcnet_vis_train/'+os.path.basename(imgdir), bbox_inches='tight',dpi=400)
#    plt.clf()
